...
def train_one(args):
    ...
    if args.model == 'vae':
        ...
        model = DDPM(T=1000, beta_start=1e-4, beta_end=0.02, base_ch=32).to(device)
        kind  = 'ddpm'
    else:
        raise ValueError(args.model)

    n_params = sum(p.numel() for p in model.parameters())
    print(f"model: {args.model}, T={args.T if kind == 'draw' else 1000}, params={n_params}")
    ...
    for epoch in range(args.epochs):
        ...
        print(f"epoch {epoch+1:02d}/{args.epochs}  train={train_loss:.4f}  test={val_loss:.4f}  ({dt:.1f}s)")
    ...
    # metric key differs: ELBO (nats) for vae/draw, simplified MSE for ddpm
    metric_name = "nelbo_nats_per_image" if kind != "ddpm" else "mse_per_pixel"
    summary = {
        "model":  args.model,
        "metric": metric_name,
...
def main():
    ...
    p.add_argument('--model', choices=['vae', 'draw_noattn', 'draw_attn', 'ddpm'], required=True)
